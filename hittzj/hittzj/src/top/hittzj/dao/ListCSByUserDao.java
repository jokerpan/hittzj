	...
	public TzjList<Commodity_Sell> getList(TzjList<Commodity_Sell> list,
		...
		try {
			...
			if (list.getTotalItemCount() < 1) {
				// 查询一共有多少个,并设置
				String hql = "select count(*) from Commodity_Sell as commodity_sell where commodity_sell.user.count="
						+ user.getCount();
			...
			String hql = "from Commodity_Sell commodity_sell where commodity_sell.user.count="
					+ user.getCount();
